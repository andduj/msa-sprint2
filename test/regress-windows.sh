...
echo "🏁 Регрессионный тест до миграции Hotelio (Windows версия)"

# Проверка соединения через Docker
echo "🧪 Проверка подключения к БД..."
docker exec hotelio-db pg_isready -U hotelio || { echo "❌ Не удалось подключиться к БД"; exit 1; }

# Загрузка фикстур через Docker
echo "🧪 Загрузка фикстур..."
docker exec -i hotelio-db psql -U hotelio -d hotelio < init-fixtures.sql
